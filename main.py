...
async def interpret_prediction_with_grok(class_id: int, confidence: float) -> str:
    """Interprète la prédiction avec Grok 3 via l'API xAI."""
    try:
        prompt = f"La prédiction de mon modèle est 'Classe #{class_id}' avec une confiance de {confidence:.2%}. Peux-tu expliquer ce que cela pourrait représenter dans un contexte réel ?"
        headers = {
        ...
        response = requests.post("https://api.x.ai/v1/grok", json=payload, headers=headers)
        response.raise_for_status()
        return response.json().get("choices", [{}])[0].get("text", "Erreur lors de l'interprétation")
    except Exception as e:
        return f"Erreur lors de l'appel à l'API Grok : {e}"
...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        return JSONResponse({
            ...
            "grok_interpretation": grok_interpretation
        })
    except Exception as e:
